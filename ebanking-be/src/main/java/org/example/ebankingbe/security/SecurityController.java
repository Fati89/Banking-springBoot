...
public class SecurityController {
    ...
    public Authentication authentication(Authentication authentication){
        return authentication; // tu renvoies toutes les infos de l’utilisateur : username, rôles (authorities), état (authenticated ou non)
    }
    ...
    public Map<String ,String> login(String username , String password){//les parametre sont par defaut des requestParam
        ...
        Authentication authentification= authentificationManager.authenticate(
                new UsernamePasswordAuthenticationToken(username,password)
        );
        Instant instant=Instant.now();//Récupérer le temps actuel
        String scope = authentification.getAuthorities().stream()
        ...
        JwtClaimsSet jwtClaimsSet=JwtClaimsSet.builder()//Création du JWT (payload)
                ...
                .expiresAt(instant.plus(10, ChronoUnit.MINUTES))//expiration dans 10 minutes
